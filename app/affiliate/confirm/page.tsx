...
export default function AffiliateConfirmPage() {
  ...
  useEffect(() => {
    const stored = sessionStorage.getItem('affiliate_signup_data');
    if (!stored) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 成功 → メールアドレスを保存して完了画面へ
      sessionStorage.setItem('affiliate_sent_email', formData.email);
      sessionStorage.removeItem('affiliate_signup_data');
      router.push('/affiliate/signup?success=true');
    ...
    <div style={{ background: '#f8fafc', minHeight: '100vh', padding: '40px 20px', fontFamily: 'sans-serif' }}>
      <main style={{ maxWidth: '600px', margin: '0 auto', background: '#fff', padding: '36px', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
        ...
        <div style={{ border: '1px solid #e2e8f0', borderRadius: '8px', padding: '20px', marginBottom: '24px' }}>
          <h3 style={{ fontSize: '16px', marginBottom: '16px', borderBottom: '1px solid #eee', paddingBottom: '8px' }}>アフィリエイト申し込み内容</h3>
          <div style={{ display: 'grid', gap: '12px' }}>
            <div><strong>氏名</strong><br />{formData.name || '未入力'}</div>
            ...
            <div><strong>事業形態</strong><br />{formData.businessType === 'corporation' ? '法人' : '個人'}</div>
            {formData.businessType === 'corporation' && (
              <div><strong>会社名</strong><br />{formData.companyName || '未入力'}</div>
            )}
            <div><strong>インボイス番号</strong><br />{formData.invoiceNumber || '未登録'}</div>
            <div><strong>報酬タイプ</strong><br />{formData.rewardType === 'recurring' ? '継続課金型（毎月5%還元）' : '一括報酬型'}</div>
